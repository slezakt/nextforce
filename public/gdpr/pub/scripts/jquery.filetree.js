		...
		$.extend($.fn, {
				fileTree: function(o) {
					...
					$(this).each( function() {
							...
							function bindTree(t) {
								var oldrel = "";
								/*$(t).find('LI A.link').hoverIntent({
									timeout: 500,
									over: function (){ $(this).parent().find('DIV.toolbar').show();},
									out: function (){ $(this).parent().find('DIV.toolbar').hide();}
									});
								*/							
								$(t).find('LI.directory A.del').bind('click', function() {
									return confirm('Opravdu chcete smazat adresář?');
								});			
								$(t).find('LI.file A.del').bind('click', function() {
									return confirm('Opravdu chcete smazat soubor?');
								});
								$(t).find('LI A.link').bind(o.folderEvent, function() {
								...
								if( o.dragAndDrop ) {
									...
									dirs = $(t).find('LI.directory > A.link');
									...
									dirs.bind( "dropstart", function( event ){
											...
											if($(event.dragTarget).hasClass('directory') && 
												$(this).parents('li.directory').filter( 
													function(){
														return $(this).children('a.link').attr('rel') == $(event.dragTarget).children('a.link').attr('rel') 
													} 
												).length > 0 ) {
												return false;
											}
									...
									dirs.bind( "drop", function( event ){
											...
											o.moveCallback($(event.dragTarget).children('a.link:first').attr('rel'),  $(this).attr('rel'), $(event.dragTarget).hasClass('directory') ? true : false );
											if( $(this).parent().hasClass('collapsed') ) {
												// Expand
												if( !o.multiFolder ) {
													$(this).parent().parent().find('UL').slideUp({ duration: o.collapseSpeed, easing: o.collapseEasing });
								...
								if( o.folderEvent.toLowerCase != 'click' ) $(t).find('LI A.link').bind('click', function() { return false; });
							}
							...
							if( o.fakeTopRoot ) {
								...
								if( o.dragAndDrop ) {
									...
									fakeroot.bind( "drop", function( event ){
											...
											o.moveCallback($(event.dragTarget).children('a.link:first').attr('rel'), null, $(event.dragTarget).hasClass('directory') ? true : false );
											// if there was a drop, move some data...
