...
class ZFDebug_Controller_Plugin_Debug extends Zend_Controller_Plugin_Abstract
{
    ...
    protected function _headerOutput() {
        ...
        return ('
            <style type="text/css" media="screen">
                ...
                #ZFDebug_debug img { display:inline-block; }
                ...
                .ZFDebug_span { border: 1px solid #999; border-right:0px; background:#DFDFDF; padding: 5px 5px; margin:0; float:none; }
                ...
                #ZFDebug_exception { border:1px solid #CD0A0A; }
            </style>
    ...
    protected function _output($html)
    {
        ...
        // search for javascript in content-type
        $doOutput = true;
        foreach ($headers as $header) {
            if (stripos($header['name'], 'content-type') !== false &&
                stripos($header['value'], 'javascript') !== false) {
                $doOutput = false;
            }
        }

        if ($doOutput) {
            //$response->setBody(preg_replace('/(<head.*>)/i', '$1' . $this->_headerOutput(), $body));
            $response->setBody(str_ireplace('</body>', $this->_headerOutput() . '<div id="ZFDebug_debug">'.$html.'</div></body>', $body));
        }
    }
}