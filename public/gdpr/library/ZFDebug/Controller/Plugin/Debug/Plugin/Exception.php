...
class ZFDebug_Controller_Plugin_Debug_Plugin_Exception implements ZFDebug_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function getTab ()
    {
        ...
        $errorCount = count(Phc_ErrorLog::$_errors);
    ...
    public function getPanel ()
    {
        ...
        if ($errorCount) {
            ...
            foreach (Phc_ErrorLog::$_errors as $error) {
                $html .= '<li>' . sprintf("%s: %s in %s on line %d", Phc_ErrorLog::_errorCodeToZendLogCode($error['type'], true), $error['message'], str_replace($_SERVER['DOCUMENT_ROOT'], '', $error['file']), $error['line']) . '</li>';
            }
            $html .= '</ol>';
        }
        return $html;
    ...
    public static function errorHandler ($level, $message, $file, $line)
    {
       /* if (! ($level & error_reporting()))
        ...
        Phc_ErrorLog::getLogger()->log(sprintf("%s: %s in %s on line %d", $type, $message, $file, $line), Zend_Log::DEBUG);
