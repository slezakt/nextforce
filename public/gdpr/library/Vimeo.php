...
class Vimeo
{
    ...
    public function upload ($file_path, $upgrade_to_1080 = false, $machine_id = null)
    {
        ...
        $response = $this->request('/me', array(), 'GET');
        if ($response['body']['upload_quota']['space']['free'] < $file_size) {
            throw new VimeoUploadException('Not enough space for uploading file because of limit restrictions.');
        }
    ...
    */
    private function perform_upload($file_path, $ticket)
    {
        if ($ticket['status'] != 201) {
            throw new VimeoUploadException('Unable to get an upload ticket.'.print_r($ticket,TRUE));
        }
    ...
    /**
     * set album for video. creates album if not exists
     *
     * @param $video_uri string uri for video
     * @param $album_name string name of album
     * @param $album_description string|null description of album
     * @return array
     */
    public function setVideoAlbum($video_uri, $album_name, $album_description = null) {

        // get video
        $response = $this->request($video_uri, array(), 'GET');
        if (!in_array($response['status'], array(200,201,204))) {
            return $response;
        }
        // extract user uri from video
        $user_uri = $response['body']['user']['uri'];

        // get all albums, find by album name, get album_id
        $album_uri = null;
        $req = $user_uri . '/albums';
        $response = $this->request($req, array(), 'GET');
        if (!in_array($response['status'], array(200,201,204))) {
            return $response;
        }
        ...
        // create album if no album with given name was found
        if (!$album_uri) {
            $req = $user_uri . '/albums';
            $response = $this->request($req, array(
                'name' => $album_name,
            ...
            // get album uri
            $album_uri = $response['body']['uri'];
        }

        // set album
        $req = $album_uri . $video_uri;
        $response = $this->request($req, array(), 'PUT');
        if (!in_array($response['status'], array(200,201,204))) {
            return $response;
        }
    ...
    /**
     * set preset for video player
     *
     * @param $video_uri
     * @param $preset_id
     * @return array
     */
    public function setVideoPreset($video_uri, $preset_id) {

        $req = $video_uri.'/presets/'.$preset_id;
        $response = $this->request($req, array(), 'PUT');
     ...
     * @param $name
     * @param $description
     * @param $license string license default is "no creative common license"
     * @param $privacy_view string default is "disable" (anybody, nobody, contacts, passwords, users, disable)
     * @param $privacy_embed string default is "private" (public, private, whitelist)
     * @param $review_link string enable review link, default is "false"
     * @return array
     */
    public function setVideoMetadata($video_uri, $name, $description,
                                     $license = '', $privacy_view = '', $privacy_embed = 'private', $review_link = 'false') {
        $req = $video_uri;
        $response = $this->request($req, array(
            'name' => $name,
