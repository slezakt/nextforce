...
abstract class elFinderVolumeDriver {
	...
	public function extract($hash) {
		...
		$this->clearcache();
		return ($path = $this->convEncOut($this->_extract($this->convEncIn($path), $archiver,$file['mime']))) ? $this->stat($path) : false;
	}
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function remove($path, $force = true) {
        

        if (preg_match('/.+\.shadow$/', $path)) {
            return true;
        }
