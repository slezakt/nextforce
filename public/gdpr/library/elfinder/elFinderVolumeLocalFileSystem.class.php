...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver {
    ...
    /**
     * Nastavi prava k souborum a slozkam
     * @param string $path cesta
     */
    public function rchmod($path) {
        
        if(is_dir($path)) {
           chmod($path,Utils::DIR_PERMISSION);
        } else {
           chmod($path,Utils::FILE_PERMISSION);
        }
        ...
        foreach ($it as $fileinfo) {
            $realpath =  rtrim($path, '\\/') .'/'. ltrim($it->getSubPathName(),'\\/');
            if ($fileinfo->isDir()) {
                chmod($realpath,Utils::DIR_PERMISSION);
            } elseif ($fileinfo->isFile()) {
                chmod($realpath,Utils::FILE_PERMISSION);
            }
        }
	...
	public function __construct() {
		...
		$this->options['dirMode']  = Utils::DIR_PERMISSION;            // new dirs mode
		$this->options['fileMode'] = Utils::FILE_PERMISSION;            // new files mode
	...
	protected function _move($source, $targetDir, $name) {
        ...
        if (Files::hasShadowFile($source)) {
            $res = rename(Files::getShadowFile($source), $target.'.shadow');
            if (!$res) return false;
        }
	...
	protected function _unlink($path) {
        if (Files::hasShadowFile($path)) {
            @unlink(Files::getShadowFile($path));
        }
		return @unlink($path);
	}
	 ...
	 * @author Alexey Sukhotin
	 **/
	protected function _extract($path, $arc,$mime = null) {
        ...
        if ($mime == "application/zip") {
            ...
            if ($zip->open($path) === true) {
                ...
                for ($i = 0; $i < $zip->numFiles; $i++) {
                    ...
                    $p = rtrim($target, '\\/') . DIRECTORY_SEPARATOR . $stat['name'];
                    
                    // delete corresponding shadow files if exists
                    if (Files::hasShadowFile($p)) {
                        unlink(Files::getShadowFile($p));
                    }
                }
                ...
                unlink($path);
                return $this->setError("Soubor se nepodařilo rozbalit.");
            }
        }

        if ($mime == "application/x-gzip" || $mime == "application/x-tar" || $mime == "application/x-bzip2") {
        
            $archive = new PharData($path);
            $it = new RecursiveIteratorIterator($archive, RecursiveIteratorIterator::SELF_FIRST);

            foreach ($it as $fileinfo) {
                if ($fileinfo->isFile()) {
                    $p = rtrim($target, '\\/') . DIRECTORY_SEPARATOR . $it->getSubPathName();
                    // delete corresponding shadow files if exists
                    if (Files::hasShadowFile($p)) {
                        unlink(Files::getShadowFile($p));
                    }
