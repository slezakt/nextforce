...
class elFinder {
	...
	protected function file($args) {
        ...
        $furl = $volume->decode($target);
        if(Files::hasShadowFile($furl)) {
            $shadow = Files::getShadowFile($furl);
            $target = $volume->encode($shadow);
        }
        ...
        if($shadow) {
             $file['name'] = str_replace('.shadow','',basename($shadow));
        }
    ...
    /**
     * Stazeni slozky jako zip
     * @param type $args
     */
    protected function downall($args) {
        $volume = $this->volume($args['folder']);
        $target = $volume->decode($args['folder']);

        $zipfile = $volume->options['startPath'].DIRECTORY_SEPARATOR.$volume->options['quarantine'].DIRECTORY_SEPARATOR.basename($target).'.zip';

        $zip = new ZipArchive();
        $zipfile = tempnam("tmp", "zip");

        if ($zip->open($zipfile, ZipArchive::CREATE)!==TRUE) {
            exit("cannot open <$target>\n");
        }
        ...
        foreach ($it as $f) {
            if (!in_array($it->getSubPath(), array('.tmb', '.quarantine'))) {
                if ($f->isFile()) {

                    if (preg_match('/' . preg_quote(Files::$shadowFileIdentifier) . '$/', $it->getSubPathname())) {
                        continue;
                    }

                    $realpath = rtrim($target, '\\/') . '/' . ltrim($it->getSubPathName(), '\\/');

                    if (Files::hasShadowFile($realpath)) {
                        $realpath = Files::getShadowFile($realpath);
                    }

                    $zip->addFile($realpath, $it->getSubPathname());
                } elseif ($f->isDir()) {
                    if (preg_match('/^\..+/', basename($it->getSubPathname()))) {
                        continue;
                    }
