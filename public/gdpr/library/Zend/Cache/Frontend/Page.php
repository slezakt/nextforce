...
class Zend_Cache_Frontend_Page extends Zend_Cache_Core
{
    ...
    public function start($id = false, $doNotDie = false)
    {
        ...
        if ($array !== false) {
            ...
            // PHC CHANGE
            // If the If-Modified-Since header is set, send 304 Not changed
            if(!empty($this->_specificOptions['http_conditional']) && isset($_SERVER['HTTP_IF_MODIFIED_SINCE'])
                && !empty($array['headers']['last-modified'][1]))
            {
                $ifModifiedSince = strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']);
                $lastModified = strtotime($array['headers']['last-modified'][1]);
