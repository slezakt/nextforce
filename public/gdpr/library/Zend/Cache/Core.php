...
class Zend_Cache_Core
{
     ...
     * 
     * PHC CHANGE
     * If request contains Pragma: no-cache or Cache-Control:no-cache header false is returned.
     * no-cache is ignored for POST request because browsers give no-cache for everz POST request
     *
    ...
    public function load($id, $doNotTestCacheValidity = false, $doNotUnserialize = false)
    {
        ...
        if( empty($_POST) && // browsers send always no-cache with POST
            (isset($_SERVER['HTTP_PRAGMA']) && preg_match('/no-cache/i', $_SERVER['HTTP_PRAGMA'])) ||
            (isset($_SERVER['HTTP_CACHE_CONTROL']) && preg_match('/no-cache/i', $_SERVER['HTTP_CACHE_CONTROL'])))
        {
            return false;
        }
        // End of PHC CHANGE
